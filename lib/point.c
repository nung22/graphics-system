...
{
  float dx = p1->val[0] - p2->val[0];
  float dy = p1->val[1] - p2->val[1];
  float dz = p1->val[2] - p2->val[2];
  return sqrtf(dx * dx + dy * dy + dz * dz);
}

// Adds two points and stores the result in the third point
void point_add(Point *p1, Point *p2, Point *result)
{
  point_set3D(result, p1->val[0] + p2->val[0], p1->val[1] + p2->val[1], p1->val[2] + p2->val[2]);
}

// Scales a point by a scalar value
void point_scale(Point *p, float scale, Point *result)
{
  point_set3D(result, p->val[0] * scale, p->val[1] * scale, p->val[2] * scale);
}
